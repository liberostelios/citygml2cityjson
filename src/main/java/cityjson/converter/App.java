...
public class App {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(df.format(new Date()) + "reading " + args[0] + " into main memory");
		CityGMLInputFactory in = builder.createCityGMLInputFactory();
		CityGMLReader reader = in.createCityGMLReader(new File(args[0]));
		...
		CityJSONWriter writer = out.createCityJSONWriter(new File(args[1]));
		writer.write(cityModel);
		writer.close();
		
		System.out.println(df.format(new Date()) + "CityJSON file " + args[1] + " written");
